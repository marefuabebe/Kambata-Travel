...
function ChangeBadge({ change, mode = "percent" }: { change: number; mode?: "percent" | "points" | "none" }) {
  ...
  if (isFlat) {
    return (
      <span className="inline-flex items-center gap-1 text-[11px] font-semibold px-2 py-0.5 rounded-md bg-slate-100 dark:bg-[#0A0F1C] text-slate-500">
        <Minus size={10} /> Flat
      </span>
    );
  }
...
function ChartCard({ title, subtitle, stat, statLabel, children, loading }: any) {
  return (
    <div className={`bg-white dark:bg-[#0A0F1C] rounded-xl border border-[#E2E8F0] dark:border-[#334155] overflow-hidden ${loading ? "opacity-60 pointer-events-none" : ""}`}>
      ...
      <div className="space-y-4">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
            ...
            {TIMEFRAMES.map((tf) => (
              <button
                ...
                className={`shrink-0 min-h-[36px] px-3 text-xs font-semibold rounded-md transition-all ${
                  timeframe === tf.id
                    ? "bg-white dark:bg-[#0A0F1C] text-[#0F172A] dark:text-white shadow-sm"
          ...
          <button
            ...
            className="inline-flex items-center justify-center gap-2 min-h-[36px] px-3.5 rounded-lg border border-[#E2E8F0] dark:border-[#334155] bg-white dark:bg-[#0A0F1C] text-sm font-medium text-slate-600 dark:text-slate-300 hover:border-[#FF8C00]/40 hover:text-[#FF8C00] transition-colors disabled:opacity-40 shrink-0"
          >
            <Download size={15} />
            Export
          </button>
      ...
      {performance && (
        <section className={refreshing ? "opacity-70 transition-opacity" : ""}>
          ...
          <div className="grid grid-cols-1 xl:grid-cols-2 gap-4">
            {/* Guides */}
            <div className="bg-white dark:bg-[#0A0F1C] rounded-xl border border-[#E2E8F0] dark:border-[#334155] overflow-hidden">
              ...
              <div className="md:hidden p-4 space-y-3">
                {performance.guides.map((g: any) => (
                  <div key={g._id} className="flex items-center gap-3 p-3 rounded-lg bg-[#F8FAFC] dark:bg-[#0F172A]">
                    <div className="w-9 h-9 rounded-full bg-white dark:bg-[#0A0F1C] overflow-hidden shrink-0">
