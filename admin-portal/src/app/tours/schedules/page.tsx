...
export default function ManageSchedulesPage() {
  ...
  useEffect(() => {
    const fetchTours = async () => {
      try {
        const { data } = await apiClient.get("/tours?limit=100");
        setTours(data.data || []);
      } catch {
  ...
  const handleNextToGuide = () => {
    if (!selectedTourId || !startDate || !endDate || !startTime || !endTime || !meetingPoint) {
  ...
  const handleAssignSchedule = async () => {
    if (!selectedTourId || !selectedGuideId || !startDate || !endDate || !meetingPoint) {
      toast.error("Tour, Guide, dates, and Meeting Point are required");
      return;
    }
    ...
    try {
      setAssigning(true);
      await apiClient.post(`/tours/${selectedTourId}/schedules`, {
      ...
      setCapacity(""); setMeetingPoint(""); setPriceOverride("");
      setAlternatives([]); setHasAvailableGuides(null);
      setWizardStep(1);
    } catch (err: any) {
  ...
  const getTourTitle = (tour: Tour) => {
    ...
    if (tour.title && tour.title.en) return tour.title.en;
    return 'Unnamed Tour';
  };
    ...
    <div className="min-h-screen bg-slate-50 dark:bg-[#0B1120] text-slate-900 dark:text-slate-50 pb-24 md:pb-12 pt-8 px-4 md:px-8 font-sans">
      <div className="max-w-[1000px] mx-auto">
        ...
        <AnimatePresence mode="wait">
            ...
            <motion.div key="step1" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }} className="bg-white dark:bg-[#131B2C] rounded-[24px] p-6 md:p-8 border border-slate-200 dark:border-white/5 shadow-sm">
              ...
              <div className="space-y-6">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-x-8 gap-y-6">
                  ...
                  <div>
                    ...
                    <div className="relative">
                      ...
                      >
                        ...
                        {tours.map(t => (
                          <option key={t._id} value={t._id} className="dark:bg-[#131B2C]">{getTourTitle(t)} (Max: {t.maxCapacity || 'N/A'})</option>
                        ))}
                      </select>
                  ...
                  {/* Meeting Point */}
                  <div>
                    <label className="block text-[13px] text-slate-700 dark:text-slate-300 mb-2 font-medium">Meeting Point *</label>
                    <div className="relative">
                      <MapPin size={18} className="absolute left-4 top-1/2 -translate-y-1/2 text-slate-400 pointer-events-none" />
                      <input 
                        type="text" 
                        placeholder="e.g. Lobby, Main Square" 
                        value={meetingPoint} 
                        onChange={e => setMeetingPoint(e.target.value)} 
                        className="w-full bg-transparent border border-slate-200 dark:border-white/10 rounded-[12px] pl-11 pr-4 py-3.5 text-sm text-slate-900 dark:text-white outline-none focus:border-[#FF8C00] font-medium transition-colors"
                      />
                    </div>
                  </div>
                </div>
                ...
                <div className="grid grid-cols-2 gap-4 md:gap-8">
                  {/* Capacity */}
                  <div>
                    <label className="block text-[13px] text-slate-700 dark:text-slate-300 mb-2 font-medium truncate">Capacity (Optional)</label>
                    <div className="relative">
          ...
          {wizardStep === 3 && selectedTour && selectedGuide && (
            <motion.div key="step3" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }} className="bg-white dark:bg-[#131B2C] rounded-[24px] p-6 md:p-8 border border-slate-200 dark:border-white/5 shadow-sm">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                ...
                <div className="bg-slate-50 dark:bg-[#1A2235] rounded-[20px] p-6 border border-slate-200 dark:border-white/5">
                  ...
                  <div className="space-y-6">
                    <div>
                      <p className="text-[11px] font-bold text-slate-400 uppercase tracking-wider mb-1">Tour</p>
                      <p className="font-bold text-sm text-slate-900 dark:text-white">{getTourTitle(selectedTour)}</p>
                    </div>
                    ...
                    <div className="grid grid-cols-2 gap-4 pt-4 border-t border-slate-200 dark:border-white/10">
                      <div>
                        <p className="text-[11px] font-bold text-slate-400 uppercase tracking-wider mb-1">Meeting Point</p>
                        <p className="font-bold text-sm text-slate-900 dark:text-white flex items-center gap-1.5"><MapPin size={14} className="text-slate-400" /> {meetingPoint}</p>
                      </div>
                      <div>
                        <p className="text-[11px] font-bold text-slate-400 uppercase tracking-wider mb-1">Capacity / Price</p>
                        <p className="font-bold text-sm text-slate-900 dark:text-white flex items-center gap-1.5"><Users size={14} className="text-slate-400" /> {capacity || selectedTour.maxCapacity}</p>
                        <p className="font-bold text-sm text-slate-500 flex items-center gap-1.5 mt-1"><DollarSign size={14} className="text-slate-400" /> {priceOverride ? `$${priceOverride}` : 'Default'}</p>
                      </div>
                    </div>
                  </div>
                </div>
