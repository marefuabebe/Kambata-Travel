    ...
    return (
      <div className="flex flex-col h-full overflow-hidden relative">
        ...
        <div className={`p-4 border-b border-gray-200 dark:border-white/10 transition-all duration-300 ${collapsed ? 'flex justify-center items-center' : ''}`}>
          <Link href={`${portalLink}/settings`} onClick={handleLinkClick} className={`flex items-center group rounded-[12px] hover:bg-gray-100/80 dark:hover:bg-white/5 transition-colors border border-transparent hover:border-gray-200 dark:hover:border-white/10 relative ${collapsed ? 'justify-center p-0' : 'gap-3 p-2'}`}>
            <div className="relative shrink-0">
              <div className="w-10 h-10 rounded-full overflow-hidden border border-gray-200 dark:border-white/10 shadow-sm relative">
                <img loading="lazy"
              ...
              {(user?.guideStatus === "approved" || user?.guideStatus === "verified") && (
                <div className="absolute -bottom-0.5 -right-0.5 w-4 h-4 bg-[#10B981] rounded-full flex items-center justify-center border-2 border-white dark:border-[#0A0F1C] shadow-sm pointer-events-none">
            ...
            {!collapsed && (
              <div className="flex flex-col overflow-hidden min-w-0">
                ...
                <span className="text-xs text-gray-500 dark:text-gray-400 truncate capitalize">
                  {user?.role === "guide" && (user?.guideStatus === "approved" || user?.guideStatus === "verified") ? (
                    <span className="text-[#10B981] font-bold">Verified Guide</span>
                  ) : (
                    user?.role || "User"
                  )}
                </span>
        ...
        <div className="flex-1 overflow-y-auto custom-scrollbar overflow-x-hidden flex flex-col">
          {/* ── Navigation ── */}
          <nav className="py-5 px-3 space-y-2">
          {navItems.map((item) => {
                ...
                <Link
                  ...
                  className={`flex items-center rounded-[12px] text-sm font-medium transition-all duration-300 relative focus:outline-none focus:ring-2 focus:ring-primary/20 ${
                    ...
                    isActive 
                      ...
                      : 'text-gray-500 dark:text-gray-400 hover:text-gray-900 dark:hover:text-white hover:bg-gray-100/80 dark:hover:bg-white/5 border border-transparent hover:border-gray-200 dark:hover:border-white/10'
                  }`}
                >
                  <Icon 
                    width={18} height={18} 
