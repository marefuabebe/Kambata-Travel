...
import VRModal from "@/components/shared/VRModal";
...
const Destinations = () => {
  ...
  const openVR = (e: React.MouseEvent, title: string) => {
    e.stopPropagation();
    // Using a beautiful open-source 360 panorama placeholder for now
    setVrImageUrl("https://pannellum.org/images/alma.jpg");
    setVrTitle(title);
    setIsVROpen(true);
  };
  ...
  const renderCard = (dest: any, isLarge = false) => (
    <div key={dest._id} className="relative group rounded-3xl overflow-hidden w-full h-full min-h-[250px] shadow-sm hover:shadow-xl transition-all duration-300 cursor-pointer">
      ...
      {/* 360 VR Button (Floating Top Right) */}
      <div className="absolute top-4 right-4 z-20">
        <button
          onClick={(e) => openVR(e, getTranslatedTitle(dest))}
          className="bg-black/50 hover:bg-primary backdrop-blur-sm text-white px-3 py-1.5 rounded-full flex items-center gap-1.5 transition-all duration-300 shadow-lg border border-white/20 group-hover:scale-110"
        >
          <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M15 12a3 3 0 11-6 0 3 3 0 016 0z" />
            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z" />
          </svg>
          <span className="text-xs font-bold tracking-wide">360°</span>
        </button>
      </div>
    </div>
  );
