  ...
  return (
    <div className="max-w-3xl mx-auto pb-16">
      ...
      <div className="mb-8">
        ...
        <div className="bg-white dark:bg-[#0A0F1C] border border-red-100 dark:border-red-500/20 shadow-sm rounded-2xl p-4 flex items-center gap-4">
      ...
      <div className="flex gap-2 mb-8 bg-gray-50 dark:bg-white/5 p-1.5 rounded-[20px]">
        {[{ id: "submit", label: "Submit SOS" }, { id: "history", label: "My Alerts" }].map((t) => (
          <button key={t.id} onClick={() => { setTab(t.id as any); if (t.id === "history") fetchAlerts(); }}
            className={`flex-1 py-3 rounded-2xl text-[14px] font-bold transition-all ${tab === t.id ? "bg-white dark:bg-[#0A0F1C] text-gray-900 dark:text-white shadow-sm" : "text-gray-500 dark:text-gray-400 hover:text-gray-700 dark:hover:text-gray-200"}`}>
            {t.label}
          </button>
        ))}
      </div>
          ...
          <motion.div key="submit" initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }}>
            {submitted ? (
              <motion.div initial={{ scale: 0.9, opacity: 0 }} animate={{ scale: 1, opacity: 1 }}
                className="bg-white dark:bg-[#0A0F1C] rounded-[2.5rem] border border-emerald-200 dark:border-emerald-500/30 shadow-2xl overflow-hidden">
        ...
        {tab === "history" && (
          <motion.div key="history" initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }}>
            ...
            {loadingAlerts ? (
              ...
              <div className="bg-white dark:bg-[#0A0F1C] rounded-[2.5rem] border border-gray-100 dark:border-white/5 p-12 text-center">
                ...
                {alerts.map((alert, idx) => {
                  ...
                  return (
                    <motion.div key={alert._id} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: idx * 0.05 }}
                      className="bg-white dark:bg-[#0A0F1C] rounded-[2rem] border border-gray-100 dark:border-white/5 shadow-sm p-6 space-y-4">
