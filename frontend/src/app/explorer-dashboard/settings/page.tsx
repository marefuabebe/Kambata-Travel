  ...
  return (
    <div className="max-w-5xl mx-auto space-y-8 pb-12 pt-4">
      ...
      <div className="lg:hidden">
        <button onClick={() => router.back()} className="flex items-center gap-2 px-4 py-2 bg-white dark:bg-[#0A0F1C] border border-gray-200 dark:border-white/10 rounded-xl text-gray-600 dark:text-gray-300 font-bold shadow-sm active:scale-95 transition-transform">
          <ChevronLeft size={18} />
          Back
        </button>
      </div>
          ...
          <div className="md:w-64 shrink-0 flex flex-col gap-2 relative">
            ...
            <div className="lg:hidden w-full relative z-50" ref={dropdownRef}>
              <button
                ...
                className="w-full flex items-center justify-between bg-white dark:bg-[#0A0F1C] backdrop-blur-xl border border-gray-200 dark:border-white/10 rounded-2xl p-4 shadow-sm"
              >
              ...
              <AnimatePresence>
                {isDropdownOpen && (
                  <motion.div
                    ...
                    className="absolute top-full left-0 right-0 mt-2 bg-white dark:bg-[#0A0F1C] border border-gray-100 dark:border-white/10 rounded-2xl shadow-xl overflow-hidden z-50 p-2 space-y-1"
                  >
                    {getTabs(t).map(tab => {
                      const Icon = tab.icon;
                      const isActive = activeTab === tab.id;
          ...
          <div className="flex-1">
            <AnimatePresence mode="wait">
              <motion.div key={activeTab}
                ...
                transition={{ duration: 0.2 }}
                className="bg-white dark:bg-[#0A0F1C] backdrop-blur-xl rounded-[2.5rem] border border-gray-100 dark:border-white/5 shadow-sm p-8 min-h-[400px]">
            ...
            <div className="mt-6 flex flex-col-reverse sm:flex-row items-center justify-between gap-4 p-6 bg-white dark:bg-[#0A0F1C] backdrop-blur-xl rounded-[2rem] border border-gray-100 dark:border-white/5 shadow-sm">
